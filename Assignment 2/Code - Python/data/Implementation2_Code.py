...
import random
script, newsgrouplabels, vocabulary, data_labels, data_data = argv #Read in files from command line


#########################################################################################################################
############ 									Take in our data files                           ########################
############ Returns: Group_Labels array, Vocab array, Data_Labels 1-D array, Data_Data 2-D array########################
def Function1(newsgrouplabels, vocabulary, data_labels, data_data):
	Group_Labels = []
	Vocab = []
	for line in open(newsgrouplabels,'r').readlines():
		Group_Labels.append(line.strip())
	
	for line in open(vocabulary,'r').readlines():
		Vocab.append(line.strip())
	
	Data_Labels = np.genfromtxt(data_labels,delimiter='')
	
	Data_Data = np.genfromtxt(data_data,delimiter='')

	return Group_Labels, Vocab, Data_Labels, Data_Data
